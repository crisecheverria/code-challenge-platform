  ...
  async (req: Request, res: Response) => {
    try {
      ...
      res.json({
        ...
        earnedBadge,
        completedChallenges: completedChallenges.map((id) => id.toString()),
      });
    } catch (error) {
...
router.get("/next-challenge", async (req, res) => {
  try {
    const { conceptTag, currentId } = req.query;
    console.log("Request params:", { conceptTag, currentId });
    ...
    if (!currentId || typeof currentId !== "string") {
      return res
        .status(400)
        .json({ error: "Current challenge ID is required" });
    }
    ...
    console.log(
      "Next challenge:",
      nextChallenge
        ? { id: nextChallenge._id, title: nextChallenge.title }
        : "None available",
    );
